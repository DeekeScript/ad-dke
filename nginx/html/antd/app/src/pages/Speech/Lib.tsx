...
import { addSpeechLib, speechLibList, removeSpeechLib, updateSpeechLib } from '@/services/api/api';
import { PlusOutlined, QuestionCircleOutlined } from '@ant-design/icons';
import { PageContainer, ProTable } from '@ant-design/pro-components';
import { Button, message, Popconfirm } from 'antd';
import React from 'react';
import UpdateForm from './components/LibUpdateForm';
import { columns } from './help/lib';
...
export default class Lib extends React.Component {
  ...
  getData = async (params: { current: number; pageSize: number; douyin: string; weixin: string; name: string; }) => {
    const res = await speechLibList(params);
    return res;
  }

  handleUpdate = async (fields: any) => {
    let res = await updateSpeechLib({ ...fields });
    if (res.success) {
    ...
    return (
      <PageContainer>
        <ProTable
          ...
          toolBarRender={() => [
            <If key={'unique_add'} access={'Api_addSpeechLib'}>
              <UpdateForm
                key={'unique__add'}
          ...
          columns={[...this.state.columns, {
            ...
            valueType: 'option',
            render: (_, record) => [
              <a key={'list_' + record.id} onClick={() => { history.push('./lib/list?id=' + record.id) }}>话术列表</a>,
              <UpdateForm
