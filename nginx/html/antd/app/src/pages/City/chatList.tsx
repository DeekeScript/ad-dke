import { focusUserChatList, removefocusUserChat } from '@/services/api/api';
import { QuestionCircleOutlined } from '@ant-design/icons';
import { PageContainer, ProTable } from '@ant-design/pro-components';
import { Popconfirm, message } from 'antd';
import React from 'react';
import { columns } from './help/focusChatList';
...
export default class List extends React.Component {
  ...
  getData = async (params: { current: number; pageSize: number; douyin: string; weixin: string; name: string; }) => {
    const res = await focusUserChatList(params);
    return res;
  }

  handleRemove = async (id: number) => {
    let res = await removefocusUserChat({
      id: id,
    });
    console.log(res);
    if (res.success) {
